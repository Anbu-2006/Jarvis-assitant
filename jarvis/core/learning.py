...
USAGE_LOG = Path(__file__).parents[1] / "api" / "data" / "usage_log.jsonl"
...
class UsageLearner:
    ...
    def get_proactive_hint(self, current_intent: str) -> Optional[str]:
        """
        Analyze usage_log.jsonl to see what usually follows current_intent.
        Returns a suggested next action description or None.
        """
        ...
        try:
            ...
            # Look for patterns (current_intent -> ?)
            followers = []
            for i in range(len(intents) - 1):
                if intents[i] == current_intent:
                    followers.append(intents[i+1])
            
            if not followers:
                return None
            
            # Most common follower
            most_common = max(set(followers), key=followers.count)
            count = followers.count(most_common)
            
            if count >= 3: # Pattern threshold
                hints = {
                    "build": "I've noticed you often run tests after a build. Shall I start them?",
                    "research": "Usually you follow up research with a project build. Ready to start?",
                    "open_terminal": "You often check your mail after opening the terminal. Want me to check it now?",
                }
                return hints.get(most_common)
                
        except Exception as e:
            log.warning(f"Proactive hint generation failed: {e}")
