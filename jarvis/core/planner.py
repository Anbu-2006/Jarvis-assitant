"""
JARVIS Task Planner — Conversational planning before spawning JARVIS.
...
from jarvis.core.templates import TEMPLATES, get_template
...
async def _classify_planning_mode_llm(
    ...
    """Use the LLM to classify request and identify missing info."""
    try:
...
class TaskPlanner:
    """Manages the planning conversation before spawning JARVIS."""
    ...
    async def build_prompt(self) -> str:
        """Build the structured LLM router prompt from the finalized plan."""
        plan = self.active_plan
        if not plan:
            return ""
    ...
    async def _classify_request(self, text: str, client: any) -> dict:
        """Use the LLM to classify request type and extract known info."""
        try:
